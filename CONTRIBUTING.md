...
**Homebrew tap自動化ステップの失敗はリリースジョブ全体を失敗表示にする**(`HOMEBREW_TAP_PAT` が未設定の場合のみステップ自体をスキップし、リリースジョブは成功のまま)。以前は全ステップに無条件で `continue-on-error: true` を付けていたため、PAT未設定以外の実際の不具合(例: v1.5.0リリースで発生した、tauri-action v1へのアップグレードに伴うリリースアセット命名規則の変更により `brew bump-cask-pr` が404で失敗した事例)も気づかれずに握りつぶされていた。リリースジョブが失敗表示になった場合は、ログの「Open a Homebrew tap update PR」以降のステップを確認すること。
