...
- [x] 4.2 実機(`cargo tauri dev` 等)で、ユーザー名/パスワード/TOTPコピーが成功する場合に、フォーカス行が点滅した後にクイックアクセスが閉じることを確認する。(2026-08-26 ユーザーによる実機確認完了)
- [x] 4.3 実機で、TOTP未設定アイテムに `⌥⌘C` を押した場合(失敗ケース)に、フォーカス行が点滅した後もクイックアクセスが閉じずに検索画面が維持されることを確認する。(2026-08-26 ユーザーによる実機確認完了)
- [x] 4.4 実機で、URL未設定アイテムに `Enter` を押した場合も同様に、点滅後クイックアクセスが維持されることを確認する。(2026-08-26 ユーザーによる実機確認完了)
- [x] 4.5 アクションメニュー(矢印キーで開く)経由でのアクション実行でも、同じ点滅+開閉挙動になることを確認する。(2026-08-26 ユーザーによる実機確認完了)
- [x] 4.6 日本語・英語それぞれの表示言語で検索ポップアップの他の文言(アンロック画面・検索結果0件時など)が引き続き正しく表示されることを確認する(回帰確認)。(2026-08-26 ユーザーによる実機確認完了)
