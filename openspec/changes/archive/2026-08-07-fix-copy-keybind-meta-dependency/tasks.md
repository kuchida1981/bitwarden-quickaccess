...
- [x] 5.2 macOS 実機(Alacritty 等、Meta キー設定を有効化していない環境)で `ctrl-o`/`ctrl-r`/`ctrl-t` による検索画面からの直接コピーが機能することを確認する
- [x] 5.3 フィールド選択画面でも同様に `ctrl-o`/`ctrl-r`/`ctrl-t` が機能し、`ctrl-p`/`ctrl-n`(選択移動)・`ctrl-u`(クエリ編集)が fzf のネイティブ動作のまま維持されていることを確認する
- [x] 5.4 対象フィールドが存在しないアイテムでキーバインド押下時のフィードバック表示を確認する
