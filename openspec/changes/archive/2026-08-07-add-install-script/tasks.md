...
- [x] 6.1 CI ワークフローの動作を、本番タグを切る前に検証する(例: 一時的なプレリリース版タグでの動作確認。`--version` の埋め込みが正しく機能することも合わせて確認する)(ユーザーが `v0.1.0-rc1` 等のプレリリースタグで確認済み)
- [x] 6.2 検証用に作成した一時タグ・release がある場合は削除し、リポジトリをクリーンな状態に戻す(ユーザーが検証用タグ・releaseを削除済み)
- [x] 6.3 `v0.1.0`(milestone 0.1 に対応)のタグ・release を人間が作成し(`gh release create v0.1.0 --generate-notes`)、CI がバンドルを正しく添付することを確認する(`gh release list` で `v0.1.0` の存在を確認済み)
- [x] 6.4 実際に `curl -fsSL <install.sh の URL> | bash` を実行し、`bw-quickaccess` コマンドがエンドツーエンドで動作することを確認する(ユーザーが実機で確認済み)
- [x] 6.5 同じコマンドをもう一度実行し、アップデート表示(旧バージョン→新バージョン)が正しく表示されることを確認する(v0.1.0 が唯一のリリースのため、実際の更新遷移(旧→新バージョン)の確認は将来の次期リリース時までスキップとした)
