...
- [x] 5.1 `app/src-tauri/src/popup.rs` に `pub const BACKEND_STATE_CHANGED_EVENT: &str = "backend-state-changed";` を追加する(`POPUP_SHOWN_EVENT` の定義の近くが自然)。
...
- [x] 6.1 `cargo test` を実行し、既存テストが通ることを確認する。(2026-08-27 実行、全35テスト成功、clippy警告なし)
- [x] 6.2 実機で、検索画面を開いたままトレイメニューから「今すぐロック」を選択し、ポップアップを閉じることなく即座にアンロック画面へ切り替わることを確認する(issue再現ケース)。(2026-08-27 ユーザーによる実機確認完了)
- [x] 6.3 実機で、6.2の後、ポップアップを一度閉じて再度開いても正常にアンロック画面が表示されることを確認する(回帰確認)。(2026-08-27 ユーザーによる実機確認完了)
- [x] 6.4 実機で、通常のフロー(ポップアップを閉じた状態でトレイからロック→ホットキーで開く)が引き続き正常に動作することを確認する(回帰確認)。(2026-08-27 ユーザーによる実機確認完了)
