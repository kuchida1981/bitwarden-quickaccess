...
- [x] 3.1 開発ビルドで、検索して閉じてから30秒以内に再度開くと検索文字列・結果一覧が保持され、テキストが全選択状態になることを確認する
- [x] 3.2 検索して閉じてから30秒を超えて再度開くと検索状態がクリアされることを確認する
- [x] 3.3 検索した状態で⌘Lロックし、30秒以内にアンロックしても検索状態が保持されない(空になる)ことを確認する
- [x] 3.4 検索した状態でバックエンド切断(例: `bw serve`プロセスを一時的に落とす)からの復旧後も検索状態が保持されない(空になる)ことを確認する
