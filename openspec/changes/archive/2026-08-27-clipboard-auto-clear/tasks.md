...
- [x] 4.3 実機での手動確認: (a)(b)(c) はユーザーが確認済み(`watch pbpaste` で監視。(b)はbw-quickaccessでコピー直後に他アプリから別の値をコピーし、30秒経過後もクリアされないことを確認)。(d)アイドルタイムアウト経由の自動ロックは(c)と同一の `clear_clipboard_if_owned` を呼ぶのみのため、ユーザー判断でスキップ
