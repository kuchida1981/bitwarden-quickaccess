...
- [ ] 6.3 (ユーザー作業)実機で、URIを持つアイテム・持たないアイテムそれぞれについて、一覧行にアイコン(またはプレースホルダー)が表示されることを確認する。
- [ ] 6.4 (ユーザー作業)実機で、オフライン状態(またはアイコン取得先に到達できない状態)でもクイックアクセスの検索・行フォーカス移動が問題なく行え、プレースホルダーが表示されることを確認する。
