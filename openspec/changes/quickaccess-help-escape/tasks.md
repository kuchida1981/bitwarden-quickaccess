...
- [x] 4.6 アクションメニュー表示中に `⌘/` を押すと、メニューが閉じてヘルプが表示されることを確認する(実機確認が必要)。当初ユーザー確認済みとしていたが、`/code-review` で `actionMenuOpen` の分岐が `⌘/` の判定より先に評価され実際には機能していないバグが発覚(design.md 決定4違反)。修正後、ユーザーが再確認し問題なしを確認済み
