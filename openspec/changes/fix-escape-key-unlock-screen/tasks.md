...
- [x] 2.2 実機で、Vaultがロックされている状態(アンロック画面)でクイックアクセスを開き、Escapeキーでポップアップが閉じることを確認する(issue #76の再現ケース)。(2026-08-26 ユーザーによる実機確認完了)
- [x] 2.3 実機で、検索画面(アンロック済み)でEscapeキーを押すと従来通りポップアップが閉じることを確認する(回帰確認)。(2026-08-26 ユーザーによる実機確認完了)
- [x] 2.4 実機で、ヘルプオーバーレイ表示中にEscapeを押すとヘルプのみが閉じ、ポップアップ自体は表示されたままであることを確認する(回帰確認)。(2026-08-26 ユーザーによる実機確認完了)
- [x] 2.5 実機で、アクションメニュー表示中にEscapeを押すとアクションメニューのみが閉じ、検索画面に戻ることを確認する(回帰確認)。(2026-08-26 ユーザーによる実機確認完了)
