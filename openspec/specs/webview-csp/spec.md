...
アプリケーションのWebViewを明示的なContent-Security-Policy(CSP)で保護し、任意コード実行やスクリプトインジェクションのリスクを低減する。CSPは自己ホストのスクリプト・スタイルのみを許可し、アイコン画像はBitwardenアイコンサービスからのみ、TauriのIPC通信は必要なスキーム・オリジンのみを許可する。
