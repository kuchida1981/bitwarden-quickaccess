...
macOS向けGUIアプリ(Tauri製)を、コード署名・notarizationなしで配布するための手順を提供する。ユーザーはリポジトリをセルフビルドするか、CIが生成した未署名の `.app` をGitHub Releasesからダウンロードして利用できる。README はGatekeeper警告への対処と、旧TUI(`bin/bw-quickaccess`)からの移行手順を明記する。コード署名・notarizationは本capabilityのスコープ外。
